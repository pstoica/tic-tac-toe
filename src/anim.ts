...
/* ============================================================
   floating background shapes — per-axis oscillation + hue drift,
   different periods per shape so they don't move in lockstep
   ============================================================ */

export interface FloatingShapeParams {
  hue: number;
  floatAmp: number;
  floatPeriod: number;
  rotXAmp: number;
...
export interface FloatingShapeRefs {
  float: HTMLElement;
  rotX: HTMLElement;
  rotY: HTMLElement;
  rotZ: HTMLElement;
...
/* each axis animates on its OWN element. running multiple anime.js tweens on
   the same element's transform (translateY + rotateX + rotateY + rotate)
   makes the last-committed frame win and the others visibly jitter, so we
   split transforms across a nested tower of divs — one axis per layer. */
export function animateFloatingShape(refs: FloatingShapeRefs, p: FloatingShapeParams) {
  // hue drifts even under reduced motion — it's a color shift, not motion —
  // and lives on the outermost layer so all children inherit --shape-hue
  refs.float.style.setProperty('--shape-hue', String(p.hue));
  animate(refs.float, {
    '--shape-hue': [p.hue, p.hue + 360],
    duration: p.huePeriod,
    ease: 'linear',
    loop: true,
  });
  ...
  animate(refs.float, {
    translateY: [
      { to: -p.floatAmp, duration: p.floatPeriod / 2, ease: 'inOutSine' },
      { to: p.floatAmp, duration: p.floatPeriod / 2, ease: 'inOutSine' },
    ],
    loop: true,
  });
  ...
  animate(refs.rotX, {
    rotateX: [
      { to: p.rotXAmp, duration: p.rotXPeriod / 2, ease: 'inOutSine' },
      { to: -p.rotXAmp, duration: p.rotXPeriod / 2, ease: 'inOutSine' },
    ],
    loop: true,
  });
  ...
  animate(refs.rotY, {
    rotateY: [
      { to: p.rotYAmp, duration: p.rotYPeriod / 2, ease: 'inOutSine' },
      { to: -p.rotYAmp, duration: p.rotYPeriod / 2, ease: 'inOutSine' },
    ],
    loop: true,
  });
  ...
  animate(refs.rotZ, {
    rotate: [
      { to: p.rotZAmp, duration: p.rotZPeriod / 2, ease: 'inOutSine' },
      { to: -p.rotZAmp, duration: p.rotZPeriod / 2, ease: 'inOutSine' },
    ],
    loop: true,
  });
